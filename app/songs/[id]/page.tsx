...
// 音樂播放器組件
function MusicPlayer({ assetbundleName, caption, vocalType }: { 
  ...
  // 音樂 URL - 使用 sekai.best CDN
  const audioUrl = `https://storage.sekai.best/sekai-jp-assets/music/long/${assetbundleName}/${assetbundleName}.mp3`
  ...
  return (
    <div className="bg-sekai-charcoal rounded-lg p-4">
      <div className="flex items-center gap-4">
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center text-xl transition-all flex-shrink-0 ${
            ...
            loading ? 'bg-sekai-ash text-sekai-mist' :
            'bg-gold-dim/20 text-gold-soft hover:bg-gold-dim/30'
          }`}
        >
          {error ? '✕' : loading ? '...' : isPlaying ? '⏸' : '▶'}
        </button>

        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-2">
            <div className="truncate">
              <p className="text-sekai-pearl font-medium truncate">{caption}</p>
              <p className="text-xs text-sekai-mist">{vocalType}</p>
            </div>
            <p className="text-sm text-sekai-silver ml-2 flex-shrink-0">
          ...
          <input
            ...
            onChange={handleSeek}
            disabled={error || loading}
            className="w-full h-1 bg-sekai-ash rounded-full appearance-none cursor-pointer"
          />
        </div>
      </div>
      ...
      {error && (
        <div className="mt-2">
          <p className="text-xs text-red-400">無法載入音訊（可能因 CORS 限制）</p>
          <a 
            href={audioUrl} 
            target="_blank" 
...
export default function SongDetailPage() {
  ...
  const songId = params?.id ? Number(params.id) : 0
    ...
    return (
      <div className="min-h-screen pt-24 pb-12">
        <div className="container-main text-center">
          <h1 className="heading-2 text-red-400 mb-4">{error || '找不到歌曲'}</h1>
  ...
  const totalNotes = difficulties.reduce((sum, d) => sum + (d.totalNoteCount || d.noteCount || 0), 0)
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
            <div className="card overflow-hidden sticky top-24">
              <div className="relative aspect-square bg-sekai-charcoal">
                <img
                  src={getMusicJacketUrl(music.assetbundleName)}
                  alt={music.title || '歌曲封面'}
              ...
              <div className="p-6">
                <h1 className="heading-2 text-sekai-pearl mb-2">{music.title || '未知歌曲'}</h1>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {sortedDifficulties.map((diff) => {
                    const info = DIFFICULTY_INFO[diff.musicDifficulty] || { 
                      name: diff.musicDifficulty, 
                      color: 'text-sekai-silver', 
                      bg: 'bg-sekai-ash' 
                    }
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-sekai-charcoal rounded-xl p-4 text-center">
                      <div className="text-3xl font-bold text-sekai-pearl">
                        {(selectedDiff.totalNoteCount || selectedDiff.noteCount || 0).toLocaleString()}
                      </div>
                      <div className="text-sm text-sekai-mist mt-1">音符數</div>
                    </div>
                    <div className="bg-sekai-charcoal rounded-xl p-4 text-center">
                      <div className="text-3xl font-bold text-sekai-pearl">
                        {(((selectedDiff.totalNoteCount || selectedDiff.noteCount || 0) * 200) / 10000).toFixed(1)}萬
                      </div>
                      <div className="text-sm text-sekai-mist mt-1">理論分數</div>
                    </div>
                    <div className="bg-sekai-charcoal rounded-xl p-4 text-center">
                      <div className="text-3xl font-bold text-sekai-pearl">
                        {((selectedDiff.totalNoteCount || selectedDiff.noteCount || 0) / 120).toFixed(1)}
                      </div>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {sortedDifficulties.map((diff) => {
                        ...
                        const score = (diff.totalNoteCount || diff.noteCount || 0) * 200
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="text-right py-3 px-2 text-sekai-pearl">
                              {(diff.totalNoteCount || diff.noteCount || 0).toLocaleString()}
                            </td>
                            <td className="text-right py-3 px-2 text-sekai-pearl">
                ...
                <div className="space-y-3">
                  {vocals.map((vocal) => (
                    <MusicPlayer
                      ...
                      caption={vocal.caption || `版本 ${vocal.id}`}
                      vocalType={vocal.musicVocalType || ''}
                    />
                  ))}
                </div>
              </div>
